...
for folder in src/test/deca/codegen/invalid/*
do
    ...
    do  
        ...
        if  decac $testfile > $error_file 2>&1 | grep "FATAL*" $error_file # true on error
        then 
            echo "\e[32mEchec attendu\e[0m  : pour compilation :   ${testfile#*/*/*/*/*/}"
        else 
            if [ ! -f $compiledfile ]; 
            then
                echo "\e[32mEchec attendu\e[0m  : Fichier non généré : ${compiledfile#*/*/*/*/*/}"
            else 
                resultat=$(echo "eee" | ima $compiledfile) 
                if [ "$resultat" = "" ]; # true on error TO BE continued
                then 
                    echo "\e[32mEchec attendu\e[0m ima :                ${compiledfile#*/*/*/*/*/}"
                else 
                    if [ "$resultat" = "$attendu" ];
                    then 
                        echo "\e[31mSucces inattendu\e[0m :                     ${testfile#*/*/*/*/*/}"
                        success=$(($success - 1)) 
                    else 
                        echo "\e[32mEchec attendu\e[0m  : de ima :             ${testfile#*/*/*/*/*/}"
                    fi
                fi
                rm -f $compiledfile
            fi
    ...
    do  
        ...
        then 
            ...
            then
                ...
                if [ "$resultat" = "" ]; # true on error TO BE continued
                then 
                    echo "\e[31mEchec inattendu\e[0m ima :                ${compiledfile#*/*/*/*/*/}"
                else 
                    if [ "$resultat" = "$attendu" ];
                    then 
